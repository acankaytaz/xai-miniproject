...
from src.gnn_model.utils import (calculate_metrics, gen_evaluations,
                                 ...
                                 get_lp_mutag_fid,get_lp_videogames_fid, get_nodes_dict)
...
class Explainer:
    def __init__(
        ...
        self.pred_df = pd.DataFrame(
            [
                {
                    "IRI": self.my_dataset.idx_map[idx]["IRI"],
                    "idx": idx
                }
        ...
        self.dataset_function_mapping = {
            ...
            "bgs": get_lp_bgs_fid,
            "videogames": get_lp_videogames_fid,
    ...
    def _run_evo(self):
        ...
        target_dict_evo_exp, duration_evo_exp, explanation_dict_evo = train_evo(
            learning_problems=self.learning_problem_fid, kg=self.evo_kg
        )
        print(
    ...
    def create_lp(self):
        ...
        ret_pred = self.lp_function(self.gt_dict_train, self.gt_dict_test, self.idx_map)
        if isinstance(ret_pred, tuple) and len(ret_pred) == 2:
            self.learning_problem_pred, self.evo_kg = ret_pred
        else:
            self.learning_problem = ret_pred
            self.evo_kg = self.dataset
        
        ret_fid = self.lp_function(self.gnn_pred_dt_train, self.gnn_pred_dt_test, self.idx_map)
        if isinstance(ret_fid, tuple) and len(ret_fid) == 2:
            self.learning_problem_fid, _ = ret_fid
        else:
            self.learning_problem_fid = ret_fid
